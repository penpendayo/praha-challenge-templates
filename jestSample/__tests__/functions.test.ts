...
describe("asyncSumOfArray", (): void => {
  describe("自然数の配列を渡した場合", (): void => {
    ...
    ])("$argumentを渡した場合、$resultが返却される", async ({ argument, result }): Promise<void> => {
      await expect(asyncSumOfArray(argument)).resolves.toBe(result);
    });
  });
  ...
  describe("0の配列を渡した場合", (): void => {
    it("0が返却される", async (): Promise<void> => {
      await expect(asyncSumOfArray([0, 0, 0])).resolves.toBe(0);
    });
  });
  ...
  describe("配列以外を渡した場合", (): void => {
    ...
    ])("$title、エラーがスローされる", async (argument): Promise<void> => {
      // @ts-ignore
      await expect(() => asyncSumOfArray(argument)).rejects.toThrow(TypeError);
    }
    );
  });
});
...
describe("asyncSumOfArraySometimesZero", (): void => {
  it("データベースの保存が失敗した場合、0が返却される", async (): Promise<void> => {
    await expect(asyncSumOfArraySometimesZero({
      database: DatabaseFailureMock,
      numbers: [1, 2, 3],
    })).resolves.toBe(0);
  });

  it("データベースの保存に成功したとき、asyncSumOfArrayの合計値が返却される", async (): Promise<void> => {
    await expect(asyncSumOfArraySometimesZero({
...
describe("getFirstNameThrowIfLong", (): void => {
  describe("getFirstNameがmockedName(10文字)を返す場合", (): void => {
    it("maxNameLengthを10で指定するとfirstNameが返却される", async (): Promise<void> => {
      await expect(getFirstNameThrowIfLong({
        maxNameLength: 10,
  ...
  describe("getFirstNameがmockedName(10文字)を返す場合、", (): void => {
    it("maxNameLengthを10未満で指定するとエラーになる", async (): Promise<void> => {
      await expect(getFirstNameThrowIfLong({
        maxNameLength: 9,
        nameApiService: NameApiServiceMock,
      })).rejects.toThrow();
